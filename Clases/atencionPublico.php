...
class AtencionPublico extends Personal{

    public function registrarAtencionPublico($email, $contrasenia)
    {
        ...
        if ($insertarUsuario->rowCount() > 0) {
            ...
            $insertarAtencionPublico = $this->pdo->prepare("INSERT INTO atencionPublico (ID) VALUES (:usuarioID)");
            $insertarAtencionPublico->bindParam(':usuarioID', $usuarioID, PDO::PARAM_INT);
            $insertarAtencionPublico->execute();
        } else {
            return false;
        }
...
public function agregarAtencionPublico($id)
{
    ...
    if ($verificarID->rowCount() > 0) {
        ...
        $insertarGerente = $this->pdo->prepare("INSERT INTO atencionPublico(ID) VALUES (:usuarioID)");
