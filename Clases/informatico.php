...
class Informatico extends Personal
{
    public function registrarInformatico($email, $contrasenia)
    {
        ...
        if ($insertarUsuario->rowCount() > 0) {
            ...
            $insertarInformatico = $this->pdo->prepare("INSERT INTO informatico (ID) VALUES (:usuarioID)");
            $insertarInformatico->bindParam(':usuarioID', $usuarioID, PDO::PARAM_INT);
            $insertarInformatico->execute();
        } else {
            return false;
        }
    }

    public function agregarInformatico($id)
{
    ...
    if ($verificarID->rowCount() > 0) {
        ...
        $insertarGerente = $this->pdo->prepare("INSERT INTO informatico (ID) VALUES (:usuarioID)");
