...
function alerta() {
  swal(
    ...
    "Se le notificara por correo electronico si cumple los requisitos para poder registrarse. Muchas gracias por querer ser parte de SISVIANSA ",
    "success"
  );
}
  ...
  if (tipoUsuario == "web") {
    ...
    formularioEmpresa.addEventListener("submit", function (e) {
      ...
      if (!confirmarEmpresa.test(empresa.value)) {
        ...
        confirmar = true;
        empresa.classList.add("errorEmpresa");
      } else {
        empresa.classList.remove("errorEmpresa");
      }
      //https://github.com/profeluisfagundez/dgetp-utu/blob/main/programacion_web/PHP_basico/estructura_match/index.php
      ...
      if (
        parseInt(telefono.value) < 0 ||
        !confirmarTelefono.test(telefono.value)
      ) {
        warningsTelefono += "El telefono no es valido <br>";
        confirmar = true;
        telefono.classList.add("error");
      ...
      var dataWebEmpresa = new FormData(formularioEmpresa);
